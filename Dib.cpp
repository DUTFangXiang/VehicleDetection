...
CDib::CDib(CSize size, int nBitCount)   // nBitCount 指定表示颜色时要用到的位数
{
	m_hFile = NULL;
	m_hBitmap = NULL;
	m_hPalette = NULL;
	m_nBmihAlloc = m_nImageAlloc = noAlloc;
	Empty();
	ComputePaletteSize(nBitCount);         //m_nColorTableEntries
...
BOOL CDib::Draw(CDC* pDC, CPoint origin, CSize size,BOOL bIsShowLineSignalCurvce )
{
	...
	if(bIsShowLineSignalCurvce==TRUE)
		...
		::StretchDIBits(pDC->GetSafeHdc(), origin.x-35, origin.y,2.4*size.cx,2.5*size.cy,
			0, 0,m_lpBMIH->biWidth, m_lpBMIH->biHeight,
			m_lpImage, (LPBITMAPINFO) m_lpBMIH, DIB_RGB_COLORS, SRCCOPY);
	if(m_hPalette!=NULL)
...
BOOL CDib::MakePalette()
{
	...
	for(int i = 0; i < m_nColorTableEntries; i++) {
		...
		pLogPal->palPalEntry[i].peBlue = pDibQuad->rgbBlue;//已经考虑到了32位
...
BOOL CDib::CutDIB(CDC *pDC,LPRECT lpRect)
{
	if(m_lpBMIH == NULL) return FALSE;
	CDC dc;
	dc.CreateCompatibleDC(pDC);
	HBITMAP hbitmap=CreateSection(pDC);
	HBITMAP hOldBitmap;
	hOldBitmap=(HBITMAP)SelectObject(dc.GetSafeHdc(),hbitmap);
	CPen pen;
